...
public class ArtGif {
	...
	public static void main(String[] args) throws IOException {
		...
		int dots = 1000;
		int numImages = 1;
		String folderPath = "c:\\Users\\Davis\\computerArtGif\\test2\\";
		File f = new File(folderPath);
		f.mkdirs();
		// Create a bunch of images
		for (int i = 0; i < numImages; i++) {
			ArtGif a = new ArtGif(width, height, dots, getColorScheme(), true, false, new File(folderPath + "test" + i + ".gif"));
	...
	private GifSequenceWriter giffy;
	
	public ArtGif(int width, int height, int num, Color[] colorScheme, boolean tooSmall, boolean withinBounds, File write) throws IIOException, IOException {
		// Get ready to make a cool gif - ART STUFF
		ImageOutputStream output = new FileImageOutputStream(write);
		giffy = new GifSequenceWriter(output, BufferedImage.TYPE_INT_ARGB, 1, false);
		BufferedImage bi = new BufferedImage(width * 2, height * 2, BufferedImage.TYPE_INT_ARGB);
		Graphics2D ig2 = bi.createGraphics();
		ig2.setColor(Color.WHITE);
		ig2.fillRect(0, 0, width * 2, height * 2);
		...
		// Draw the points - ART STUFF
		for (Point2D.Float pt : points) {
			ig2.draw(new Rectangle2D.Float(pt.x-.5f, pt.y-.5f, 1, 1));
		}
